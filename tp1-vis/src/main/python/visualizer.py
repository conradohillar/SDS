...
def on_pick(event):
    ...
    # neighbors.txt uses 1-based IDs (from InputParser.buildParticles id = i+1)
    # so look up by idx+1 and convert each neighbor ID back to 0-based
    for nbr_id in neighbors.get(idx + 1, []):
        nbr = nbr_id - 1  # convert 1-based ID → 0-based circle index
        if 0 <= nbr < N:
